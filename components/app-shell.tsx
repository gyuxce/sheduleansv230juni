...
import { MobileNavigation, type NavigationItem } from "@/components/mobile-navigation";
...
export function AppShell({ children, orgSlug, role, organizationName, organizationId, memberId, initialNotifications, canSelfBook = false }: { children: React.ReactNode; orgSlug: string; role: Role; organizationName: string; organizationId: string; memberId: string; initialNotifications: NotificationItem[]; canSelfBook?: boolean }) {
  const items = navigation[role].filter((item) => !item.requiresSelfBook || canSelfBook) as NavigationItem[];
  return <div className="app-shell"><aside className="sidebar"><Link className="brand" href={`/${orgSlug}/${role}/dashboard`} prefetch>日本語 {organizationName}</Link><p className="sidebar-role">Login sebagai {role}</p><NotificationBell organizationId={organizationId} memberId={memberId} initialItems={initialNotifications} /><nav className="nav" aria-label="Navigasi utama">{items.map((item) => <Link key={item.path} href={`/${orgSlug}/${role}/${item.path}`} prefetch>{item.label}</Link>)}<form action={logout}><button className="button" type="submit">Keluar</button></form></nav></aside><main className="content">{children}</main><MobileNavigation orgSlug={orgSlug} role={role} items={items} /></div>;
}
